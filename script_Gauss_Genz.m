...
%
% Description: 
% Script to perform a stability and error analysis of the Gaussian RBF for Genz' test functions 
%
% Author: Jan Glaubitz 
% Date: June 22, 2021 
...
kernel = 'G'; % kernel (G, MQ, IQ, Wendland, TPS, cubic) 
...
%[p_count, d, 1, min_error, Ep(index), s(index)]
%[min_error,index] = min( err(:,4) );
%[p_count, d, 4, min_error, Ep(index), s(index)]
...
figure(5) 
p = plot( Ep,s,'b-.', Ep,err(:,1), 'k--', Ep,err(:,4),'r-'); 
line2 = yline(opt(1),':','$\|I\|_{\infty}$','Interpreter','latex'); 
set(line2, 'LineWidth',3.5, 'FontSize',26 );
set(p, 'LineWidth',3.5)
set(gca, 'FontSize', 26)  % Increasing ticks fontsize
xlim([ Ep(1); Ep(end) ]) 
xlabel('$\varepsilon$','Interpreter','latex') 
